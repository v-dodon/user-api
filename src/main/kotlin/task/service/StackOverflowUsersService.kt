...
import task.dtos.StackOverflowUser
import task.util.*
...
class StackOverflowUsersService {
    ...
    private fun getStackOverflowUsers(): List<StackOverflowUser> {
        ...
        runBlocking {
            ...
            stackOverflowUsers.addAll(users.map { user -> StackOverflowUser(user, userTags.filter { tag ->
                user.userId.equals(tag.userId) }.map { tag -> tag.name }) }
